...
#------------------------------------------------------------------------------------------------------
    ...
    def run(self):
        ...
        vessels = filter(lambda host: "vessel" in host.name, simulation.hosts)        

        # for each server
        for server in vessels:
            # We open a xterm and start the server
            self.startServer(server)

        # Uncomment this to start firefox from the first client node on startup.
